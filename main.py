...
💸Доход с рефералов 1 Уровня - 20%
💸Доход с рефералов 2 Уровня - 10%'''.format(referals,refs2nd,bot_info.username,user['user_id']),
...
💸Доход с рефералов 1 Уровня - 20%
💸Доход с рефералов 2 Уровня - 10%'''.format(user['referal'],referals,refs2nd,bot_info.username,user['user_id'],bot_info.username,user['user_id']),
                    'markup': messages.start['markup']
                }
                try:
...
💸Доход с рефералов 1 Уровня - 20%
💸Доход с рефералов 2 Уровня - 10%'''.format(
...
💸Доход с рефералов 2 Уровня - 10'''.format(
    ...
    if text == '⚠️ Правила':
     return send_message(message,{'text':'''
       ⛔️ПРАВИЛА использования бота:

 1. Отписываться от канала (в течение 10 дней).
 2. Спамить бота повторными командами.
 За несоблюдение правил баланс пользователя обнуляется в пользу запасов бота.

 Рекламадателям запрещено:

 1. После оформления заказа убирать права у бота.
 В случае обнаружения ваш заказ удаляется, деньги не возвращаются.
 Можно убрать после окончания накрутки!
    ...
    if text == 'Другой способ':
        return send_message(message,{'text':'Если вы хотите пополнить свой баланс иным способом - пожалуйста напишите администратору @MoneyGloryBot_Admin','markup':messages.start['markup']})
    if text == 'Мои заказы':
